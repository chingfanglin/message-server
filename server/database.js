...
if (!connectionString) throw new Error('must set DATABASE_URL environment var')
    ...
    connect(function(err, client, done) {
      ...
      client.query(query, params, function(err, result) {
        ...
        if (err) {
          if (err.code === '40P01') {
            console.error(
              '[INTERNAL] Warning: Retrying deadlocked transaction: ',
              query,
              params
            )
            return doIt()
  ...
  function doIt() {
    connect(function(err, client, done) {
      ...
      function rollback(err) {
        ...
        if (err.code === '40P01') {
          console.error(
            '[INTERNAL_ERROR] Warning: Retrying deadlocked transaction..'
          )
          return doIt()
        }
    ...
    client.query(
      ...
      function(err, data) {
        ...
        client.query(
          ...
          function(err, data) {
            if (err) {
              if (err.code === '23505') return callback('USERNAME_TAKEN')
...
exports.getUserById = function(username, callback) {
  getClient(function(client, callback) {
    client.query(
      'SELECT id, username FROM users WHERE lower(username) = lower($1) AND status = true',
      [username],
      function(err, data) {
        if (err) return callback(err)
        if (!data.rows) return callback('NO_USER')
...
exports.joinChannel = function(channelName, users, callback) {
  getClient(function(client, callback) {
    ...
    client.query('SELECT id FROM channel WHERE users = $1', [users], function(
      err,
      data
    ) {
      if (data.rows.length > 0) {
      ...
      const uuid = uuidv4()
      client.query(
        'INSERT INTO channel(id, name, users) VALUES($1, $2, $3) RETURNING id',
        [uuid, channelName, users],
        function(err, data) {
          if (err) return callback(err)
          console.log(uuid)
...
exports.sendMessage = function(user_id, token, channel, message, callback) {
  getClient(function(client, callback) {
    client.query(
      'SELECT mfa_secret FROM users WHERE id = $1 AND status = true',
      [user_id],
      function(err, data) {
        if (err) return callback(err)
        if (data.rows.length === 0) return callback('NO_USER')
        ...
        try {
          ...
          client.query(
            'INSERT INTO chat_messages(user_id, message, channel) VALUES($1, $2, $3) RETURNING id',
  ...
  getClient(function(client, callback) {
    client.query(
      ...
      function(err, data) {
        ...
        if (data.rows.length === 0) return callback('NO_FROM_USER')
        ...
        try {
          ...
          client.query(
            'SELECT username FROM users WHERE id = $1 AND status = true',
            [to_user_id],
            function(err, data) {
              if (err) return callback(err)

              if (data.rows.length === 0) return callback('NO_TO_USER')

              const uuid = uuidv4()
              client.query(
                'INSERT INTO friends(id, from_user_id, to_user_id) VALUES($1, $2, $3) RETURNING id',
                [uuid, from_user_id, to_user_id],
                function(err, data) {
                  if (err) return callback(err)
    ...
    client.query(
      ...
      function(err, data) {
        ...
        try {
          ...
          client.query(
            'DELETE FROM friends WHERE from_user_id = $1 AND to_user_id = $2 RETURNING *',
    ...
    client.query(
      ...
      function(err, data) {
        ...
        try {
          ...
          client.query(
            'UPDATE friends SET is_agree = $1 WHERE id = $2 RETURNING from_user_id',
            [true, friends_id],
            function(err, data) {
              if (err) return callback(err)
              const uuid = uuidv4()
              ...
              client.query(
                'INSERT INTO friends(id, from_user_id, to_user_id, is_agree) VALUES($1, $2, $3, $4) RETURNING id',
                [uuid, user_id, friends.from_user_id, true],
                function(err, data) {
                  if (err) return callback(err)
                  return callback(null, data)
                }
    ...
    client.query(
      ...
      function(err, data) {
        ...
        try {
          ...
          client.query(
            'UPDATE friends SET is_block = $1 WHERE id = $2 RETURNING from_user_id',
            [true, friends_id],
            function(err, data) {
              if (err) return callback(err)
